...
/**
 * Favourite List functionality Controller. Performs operation for list, search
 * and delete operations of Favourite
 * 
 * @author priya mandal
 */
@WebServlet(name = "FavouriteListCtl", urlPatterns = { "/ctl/FavouriteListCtl" })
public class FavouriteListCtl extends BaseCtl {

	private static Logger log = Logger.getLogger(FavouriteListCtl.class);
	...
	protected void preload(HttpServletRequest request) {
		...
		FavouriteModel tmodel = new FavouriteModel();

		try {

			List flist = tmodel.list(0, 0);

			request.setAttribute("AddedDate", flist);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("FavouriteListCtl doGet Start");
		List list = null;
		List nextList = null;
...
//	        get the selected checkbox ids array for delete list
		...
		log.debug("FavouriteListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		log.debug("FavouriteListCtl doPost Start");
		...
		if (OP_SEARCH.equalsIgnoreCase(op)) {
			...
			if (ids != null && ids.length > 0) {
				FavouriteBean deletebean = new FavouriteBean();
				for (String id : ids) {
					...
					ServletUtility.setSuccessMessage("Favourite is Deleted Successfully", request);
				}
			} else {
	...
	protected String getView() {
		return ORSView.FAVOURITE_LIST_VIEW;
	}
